...
module.exports = {
  ...
  createJob: function(owner, job_name, messages_key, target_number, frequency, callback) {
    var job = {owner: owner, job_name: job_name, messages_key: messages_key, target_number: target_number, frequency: frequency};
  ...
  findJob: function(user_id, callback) {
    db.findOne('jobs', {owner: user_id}, function(err, job) {
      if (job)
        callback(err, job);
      else
        callback(err, null);
