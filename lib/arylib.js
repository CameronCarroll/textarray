...
var bcrypt = require('bcrypt'),
    ...
    kuejobs = kue.createQueue(),
    request = require('request').post,
    url = require('url');

kue.redis.createClient = function() {
  var our_url;
  if (process.env.MYREDIS_URL)
    our_url = process.env.MYREDIS_URL
  else
    our_url = 'redis://localhost:6379'

  console.log('myredis: ' + process.env.MYREDIS_URL);
...
module.exports = {
  ...
  checkAndQueueMessages: function() {
    db.findPendingJobs(function(err, pending_jobs) {
      if ( 0 < pending_jobs.length) { // if there are pending jobs
